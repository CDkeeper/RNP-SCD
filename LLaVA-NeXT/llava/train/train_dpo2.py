...
def process_video(video_file, target_frames=64):
    ...
    frame_diffs = []
    min_interval = max(1, int(total_frame_num / target_frames))
    last_frame = vr[0].asnumpy()
    for i in range(1, total_frame_num):
        frame = vr[i].asnumpy()
        frame_diff = calculate_histogram_difference(last_frame, frame)
        frame_diffs.append((i, frame_diff))
        last_frame = frame
    ...
    # 选择关键帧
    key_frames = []
    last_selected = -min_interval

    for idx, diff in frame_diffs:
        if len(key_frames) >= target_frames:
            break
        if idx - last_selected >= min_interval:
            # 确保新选择的帧与之前所有已选择的关键帧之间的间隔都至少为min_interval
            if all(abs(idx - kf) >= min_interval for kf in key_frames):
                key_frames.append(idx)
                last_selected = idx
    main_key_frames = key_frames.copy()
    # 如果关键帧数量不足，可以通过线性插值补充
    if len(key_frames) < target_frames:
        # 计算需要补充的帧数
        num_to_add = target_frames - len(key_frames)
        # 生成一个线性分布的帧索引
        linear_frames = np.linspace(0, total_frame_num - 1, num_to_add, dtype=int).tolist()
        # 过滤掉已经在关键帧列表中的帧索引，并确保新帧与关键帧之间的间隔
        linear_frames = [f for f in linear_frames if f not in key_frames and all(abs(f - kf) >= min_interval for kf in key_frames)]
        # 将补充的帧索引添加到关键帧列表中
        key_frames.extend(linear_frames)

    if len(key_frames) < target_frames:
        # 如果还不足，从总帧数中删除已经选取的帧数
        remaining_frames = set(range(total_frame_num)) - set(key_frames)
        remaining_frames = sorted(list(remaining_frames))
        remaining_frame_num = len(remaining_frames)
        
        # 按照剩余的总帧数除以需要的帧数，每这个间隔取一帧
        if remaining_frame_num > 0:
            new_interval = max(1, int(remaining_frame_num / (target_frames - len(key_frames))))
            # 生成新的关键帧索引，确保数量不超过所需数量
            new_key_frames = list(range(0, remaining_frame_num, new_interval))[:target_frames - len(key_frames)]
            # 将新选取的帧索引映射回原始帧索引
            new_key_frames = [remaining_frames[idx] for idx in new_key_frames]
            key_frames.extend(new_key_frames)

    # print(f"min_interval:{min_interval},main_key:{main_key_frames},num_to_add:{linear_frames},remaining_to_add:{new_key_frames},frame_idx:{key_frames}")
    key_frames.sort()
        ...
        if "image" in sources[0]:
            ...
            if suffix == "pkl":
                ...
                if "shareVideoGPTV" not in video_file and "liangke" not in video_file:
                    frame_idx = process_video(video_file=video_file, target_frames=self.data_args.frames_upbound)
                    vr = VideoReader(video_file, ctx=cpu(0))
                    # total_frame_num = len(vr)
                    # avg_fps = round(vr.get_avg_fps() / self.data_args.video_fps)
                    # frame_idx = [i for i in range(0, total_frame_num, avg_fps)]
                    # print(f"avg_fps{avg_fps},and frame_idx:{frame_idx}")
                    if self.data_args.frames_upbound > 0:
                        if len(frame_idx) > self.data_args.frames_upbound:
                            print("error!,frame_idx > frames_upbound!!!!!!!!!!")
                            # uniform_sampled_frames = np.linspace(0, total_frame_num - 1, self.data_args.frames_upbound, dtype=int)
                            # frame_idx = uniform_sampled_frames.tolist()
                    # with autocast():
                    #     video = vr.get_batch(frame_idx).asnumpy()
                    #     video = np.array(video, dtype=np.float16)  # Convert to half precision to save memory                       
                    video = vr.get_batch(frame_idx).asnumpy()  #视频精度为uint8
